...
fn test_escape_sequences() {
    ...
    if let JsonValue::Object(map) = res {
        ...
        assert_eq!(
            map.get("backslash").unwrap(),
            &JsonValue::String("\\".to_string())
        );
        assert_eq!(
            map.get("slash").unwrap(),
            &JsonValue::String("/".to_string())
        );
        assert_eq!(
            map.get("backspace").unwrap(),
            &JsonValue::String("\x08".to_string())
        );
        assert_eq!(
            map.get("formfeed").unwrap(),
            &JsonValue::String("\x0c".to_string())
        );
        assert_eq!(
            map.get("newline").unwrap(),
            &JsonValue::String("\n".to_string())
        );
        assert_eq!(
            map.get("carriage").unwrap(),
        ...
        assert_eq!(
            ...
            &JsonValue::String("\u{00A9}".to_string())
        );
    } else {
...
fn test_multiple_top_level_values() {
    ...
    assert!(
        res.unwrap_err()
            .contains("Unexpected token after root value")
    );
}
